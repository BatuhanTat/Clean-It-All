...
public class Clean : MonoBehaviour
{
    ...
    private void Update()
    {
        if (GameManager.instance.canClean && !GameManager.instance.inMenu)
        {
            Cleaning();
            dirtText.text = Mathf.RoundToInt(GetDirtAmount() * 100f) + "%";
        }
    }
    ...
    private void OnSelect(InputValue value)
    {
        ...
        Debug.Log("gamemanager: " + GameManager.instance.canClean);
        if (!GameManager.instance.canClean)
        {
            cameraController.OnSelectCamera(value);
        }
    }
